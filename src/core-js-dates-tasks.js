...
function getTime(date) {
  ...
  let seconds = formatedDate.getSeconds();
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber =
    Math.round((tempDate - firstThursday) / (7 * 24 * 60 * 60 * 1000)) + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (dayCounter < countWorkDays) {
      workSchedule.push(formatEnteredDate(currentDate));
    }

    currentDate.setDate(currentDate.getDate() + 1);

    dayCounter = (dayCounter + 1) % cycleLength;
  }
